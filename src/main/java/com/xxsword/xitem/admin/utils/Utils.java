...
//        Pattern pattern = Pattern.compile("^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d).{8,}$");
    ...
    /**
     * 获取文本的token数
     * TODO 要准确计算token数量太麻烦，这里的计算方法先将就用着
     *
     * @param text
     * @return
     */
    ...
    /**
     * 去除字符串中多余的空行（连续多个空行压缩为单个空行）
     *
     * @param input 原始字符串
     * @return 处理后的字符串
     */
    public static String removeExtraEmptyLines(String input) {
        if (StringUtils.isBlank(input)) {
            return "";
        }
        // 使用正则表达式将连续2个或以上换行符替换为1个换行符
        return input.replaceAll("\\n{2,}", "\n");
    }
    public static String httpInfoDomain(HttpServletRequest request) {
        String protocol = request.getHeader("X-Forwarded-Proto"); // 返回 "http" 或 "https"
        String domain = request.getHeader("X-Forwarded-Host"); // 返回原始域名
        String port = request.getHeader("X-Forwarded-Port");
        if (protocol == null) {
            protocol = request.getScheme(); // 回退到默认协议
        }
        if (domain == null) {
            domain = request.getServerName(); // 回退到默认域名
        }
        if (port == null) {
            port = String.valueOf(request.getServerPort()); // 回退到默认端口
        }
